...
# Retrieve the best model (and best C value) from the coarse grid search
...
# Measure prediction time on the test set using the best model
...
plt.ylabel("Accuracy")
plt.title("Logistic Regression Accuracy vs. C Value for Dataset 1 (Grid Search)")
plt.legend()
plt.grid(True)
...
plt.annotate(
    ...
    xytext=(20, -50),                         # shift the text by (x=50, y=-30) points
...
inset_ax.annotate(
    ...
    xycoords='data',
    xytext=(30, 70),  # offset (in points) from the arrow tip
...
plt.savefig("logreg_accuracy_vs_C_with_refined_zoom_m1.png", bbox_inches='tight')
plt.show()
