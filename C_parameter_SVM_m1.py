...
coarse_C_values = np.logspace(-4, 3, 10)
...
grid_search = GridSearchCV(svm_model, param_grid_coarse, cv=3, scoring='accuracy',
...
refined_grid_search = GridSearchCV(svm_model, param_grid_refined, cv=5, scoring='accuracy', n_jobs=-1, verbose=1, return_train_score=True)
...
refined_overlap_accuracy = (refined_train_scores[min_diff_index_refined] + refined_test_scores[
    min_diff_index_refined]) / 2
...
ax.set_title("SVM Accuracy vs. C Value for Dataset 1 (Grid Search)")
ax.legend()
ax.grid(True)
...
plt.annotate(
    ...
    xy=(best_C_overlap, overlap_accuracy),  # arrow tip
    xycoords='data',  # interpret xy in data coords
    xytext=(10, 50),  # shift the text by (x=50, y=-30) points
    textcoords='offset points',  # interpret xytext in offset coords
    arrowprops=dict(facecolor='red', arrowstyle="->"),
    fontsize=12,
    color='red',
    zorder=10,  # bring text above other elements
    clip_on=False  # allow the annotation to extend outside the axes
)
...
inset_ax.annotate(
    ...
    xy=(refined_best_C_overlap, refined_overlap_accuracy),  # arrow tip (in data coords)
    xycoords='data',
    xytext=(10, 70),  # offset (in points) from the arrow tip
...
plt.savefig("svm_accuracy_vs_C_train_test_with_refined_zoom_m1.png", bbox_inches='tight')
plt.show()
