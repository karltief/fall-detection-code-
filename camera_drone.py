...
# Initialize Tello drone and start its video stream
tello = Tello()
tello.connect()
print("Battery Life:", tello.get_battery(), "%")
tello.streamon()
# -------------------------------------------------------------------------
...
while True:
    ...
    if results.pose_landmarks:
        ...
        cv2.putText(frame, f"Ang Vel: {ang_vel:.2f} deg/frame", (20, 430), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0, 0, 0), 1,
                    cv2.LINE_AA)
        cv2.putText(frame, f"Yaw Ang: {yaw_angle:.2f} deg", (20, 460), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0, 0, 0), 1,
                    cv2.LINE_AA)
        cv2.putText(frame, f"Distance: {dist_cam:.2f}m", (20, 400), cv2.FONT_HERSHEY_TRIPLEX, 0.6, (0, 0, 0), 1,
                    cv2.LINE_AA)
    ...
    if key == ord('t'):
        ...
        print("Moving left")
        tello.move_left(20)
    elif key == ord('dq'):  # Right arrow key
        print("Moving right")
        tello.move_right(20)
